...
namespace captive 
{
  namespace arch 
  {
    namespace aarch64 
    {
      ...
      ;
      class aarch64_cpu : public CPU 
      {
        public:
        aarch64_cpu(aarch64_environment& env, PerCPUData *per_cpu_data);
        virtual ~aarch64_cpu();
        bool init() override;
        ...
        protected:
        void __builtin_set_feature(aarch64_features feature, uint32_t value) 
        {
          feature_manager().set_feature(feature, value);
        }
        uint32_t __builtin_get_feature(aarch64_features feature) const 
        {
