		...
		namespace dbt {
			namespace util {
				...
				struct MapIteratorPair {
					...
					MapIteratorPair(const TNode& node) : _key(node.Key), _value(node.Value)
					{
					}

					const typename TNode::KeyType& _key;
					const typename TNode::ValueType& _value;

					const typename TNode::KeyType& key() const
					{
						return _key;
					}

					const typename TNode::ValueType& value() const
					{
						return _value;
					}
				};
				...
				public:
					...
					MapIterator(Support& support, const TNode *root) : _queue(support), _current(root)
					{
						if (root) {
							_queue.enqueue(root);
							advance();
				...
				public:
					...
					Map(Support& support) : _support(support), _root(nullptr), _count(0)
					{
					}
					...
					bool get(TKey const& key, TValue& value) const
					{
						Node *node = _root;
					...
					TValue assume_get(TKey const& key) const
					{
						Node *node = _root;
					...
					{
						return Iterator(_support, _root);
					}

					ConstIterator end() const
					{
						return Iterator(_support, nullptr);
					}
