...
#define UNIMPLEMENTED do { dump(); fatal("*** UNIMPLEMENTED %s:%d ***\n", __func__, __LINE__); } while(0)
...
X86Emitter::X86Emitter(const ArchData& arch_data, Context& context, Block *current_block, bool no_mmu, bool kernel_mode) : Emitter(context, current_block), _arch_data(arch_data), _no_mmu(no_mmu), _kernel_mode(kernel_mode), _x86_context((X86Context&) context), _used_features(0)
{
}
...
void X86Emitter::debug_print_operand(X86Value* op)
{
	...
	case X86ValueKind::BINARY_OPERATION:
		...
		case X86BinaryOperationKind::ADD: printf("+ ");
			break;
		default: printf("? ");
			break;
		}
	...
	case X86ValueKind::CAST:
	{
		...
		case X86CastKind::SX: printf("sx ");
			break;
		case X86CastKind::ZX: printf("zx ");
			break;
		case X86CastKind::TRUNCATE: printf("tr ");
			break;
		case X86CastKind::REINTERPRET: printf("ri ");
			break;
		case X86CastKind::CONVERT: printf("cvt ");
			break;
		}
	...
	case X86ValueKind::CONSTANT: printf("%lx:%u ", ((X86Constant *) op)->value(), ((X86Constant *) op)->width_in_bits());
		break;
	case X86ValueKind::GUEST_REGISTER: printf("greg(%lx) ", ((X86GuestRegister *) op)->offset());
		break;
	case X86ValueKind::LOCAL_VARIABLE: printf("%%%s%u ", ((X86LocalVariable *) op)->reg().name(), ((X86LocalVariable *) op)->reg().unique_index());
		break;
	case X86ValueKind::PC_REGISTER: printf("pc ");
		break;
	case X86ValueKind::UNARY_OPERATION: printf("unop ");
		break;
	case X86ValueKind::UNKNOWN: printf("<unknown> ");
		break;
	default: break;
	}
}
...
{
	uint32_t feature_mask = (1u << 30) | 7; //_used_features;
...
captive::arch::dbt::mc::x86::Instruction *X86Emitter::add_mov_auto(const captive::arch::dbt::mc::x86::Operand& src, const captive::arch::dbt::mc::x86::Operand& dst)
{
	...
	if (src.is_reg() && dst.is_reg()) {
		...
		if (src.reg.reg.is_gpr()) {
			...
			case 32: mov_kind = InstructionKind::MOVD;
				msrc.reg.width = 128;
				break;
			case 64: mov_kind = InstructionKind::MOVQ;
				msrc.reg.width = 128;
				break;
			case 128: assert(src.reg.width == 128);
				mov_kind = InstructionKind::MOVDQA;
				break;
		...
		if (dst.reg.reg.is_gpr()) {
			...
			case 64:
				...
				mdst.reg.width = 128;
				break;
			case 128: assert(dst.reg.width == 128);
				mov_kind = InstructionKind::MOVDQA;
				break;
			default: UNIMPLEMENTED;
			}
